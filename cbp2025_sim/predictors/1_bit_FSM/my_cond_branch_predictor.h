...
class SampleCondPredictor
{
    ...
    struct BTBEntry {
        bool valid;
        uint32_t tag;
        uint8_t state;   // 1-bit counter (0 = NT, 1 = T)
    };
...
public:
    ...
    void setup()
    {
        for(int i = 0; i < NUM_SETS; i++)
        {
            ...
            for(int w = 0; w < WAYS; w++)
            {
                ...
                table[i][w].state = 0;   // Initialize to Not Taken
            }
        }
    }
                ...
                bool resolveDir, bool predDir, uint64_t nextPC)
    {
        ...
        else
        {
            ...
            entry->valid = true;
            entry->tag = tag;
            entry->state = 0;  // Initialize to Not Taken

            lru[index] = victim ? 0 : 1;
        }

        // 1-bit update: just remember last outcome
        entry->state = resolveDir ? 1 : 0;
    }
};
